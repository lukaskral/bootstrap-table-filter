!function ($) {
    ...
    BootstrapTableFilter.DEFAULTS = {
      ...
      onFilterChanged: function(data) { return false; },
      onResetView: function() { return false; },
      onAddFilter: function(filter) { return false; },
      onRemoveFilter: function(field) { return false; },
      onEnableFilter: function(field) { return false; },
      onDisableFilter: function(field) { return false; },
      onSelectFilterOption: function(field, option, data) { return false; },
      onUnselectFilterOption: function(field, option) { return false; },
      onDataChanged: function(data) { return false; },
      onSubmit: function(data) { return false; },
    };
    ...
    BootstrapTableFilter.FILTER_SOURCES = {
      int: {
        search: false,
        rows: [
          {id: 'lte', label: 'Less than <input type="text">'},
          {id: 'gte', label: 'More than <input type="text">'},
          {id: 'eq', label: 'Equals <input type="text">'}
        ]
      },
      float: {extend: 'int'},
      text: {
      ...
      select: {
        ...
        rowsCallback: function(filter, searchPhrase) {
          ...
          if (searchPhrase.length) {
            vals = vals.filter(function(el) { return rowLabel(el).indexOf(searchPhrase) > -1 });
          }
          this.clearFilterOptions(filter.field);
          this.fillFilterOptions(filter.field, vals);
        }
      }
    };
    ...
    BootstrapTableFilter.prototype.initFilters = function () {
      ...
      this.$toolbar.delegate('.remove-filters *', 'click', function() {
        $.each(that.filters, function(i, filter) { that.disableFilter(filter.field) });
      });
    };
    ...
    BootstrapTableFilter.prototype.unselectFilterOption = function (field, option) {
      ...
      if (typeof filter.selectedOptions !== 'undefined' && typeof filter.selectedOptions._values !== 'undefined') {
        filter.selectedOptions._values = filter.selectedOptions._values.filter(function(item) { return item != option });
        if (filter.selectedOptions._values.length == 0) delete filter.selectedOptions._values;
      }
    ...
    $.fn.bootstrapTableFilter = function (option, _relatedTarget) {
        ...
        this.each(function () {
            var $this = $(this),
                ...
                options = $.extend({}, BootstrapTableFilter.DEFAULTS, $this.data(),
